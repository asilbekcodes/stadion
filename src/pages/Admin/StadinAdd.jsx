...
const StadinAdd = ({ stadionCount }) => {
  ...
  const deleteStadion = async (item) => {
    setLoading(true);
    try {
      await axios.delete(`${baseUrl}stadion/admin-stadion-delete/${item.id}/`, Adminconfig);
      getMalumotlar();
    } catch (err) {
          ...
          <div className="hidden sm:block relative overflow-x-auto shadow-md sm:rounded-lg">
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={5} className="text-center p-5">Yuklanmoqda...</td>
                  </tr>
                ) : getMalumot.length > 0 ? (
                  getMalumot.map((item) => (
                    <tr key={item.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                      ...
                      <td className="px-6 py-4">{item.description.length > 30 ? item.description.slice(0, 30) + "..." : item.description}</td>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-5 text-center text-gray-500 dark:text-gray-400">
